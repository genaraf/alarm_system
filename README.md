...
- публикует оставшееся время в формате `HH:MM:SS`;
...
- `V1` (`String`): отображение оставшегося времени `HH:MM:SS`.

UI для человека:
- `V2` (`Integer` или `Enumerable`): preset menu `0..5`
- `V5` (`Integer`): кнопка `Start`
- `V6` (`Integer`): кнопка `Stop`
- `V7` (`Integer`, опционально): состояние сирены, `1` или `0`
...
- кнопку `Start` на `V5` в режиме `Push`
- кнопку `Stop` на `V6` в режиме `Push`
- для обеих кнопок включите подтверждение (`confirmation`)
- при необходимости добавьте `LED` или `State` на `V7`
